...
def build_create_offset_plan(items):
    ...
    for item in items:
        ...
        for node in data["objects"]:
            plan.append({"object":node,"group_name":node+(data["extra_name"] or "ExtraName")})
    return tuple(plan)
...
def execute_create_offset_plan(plan,insert_offset_group_fn=None,exists_fn=None):
    ...
    for entry in tuple(plan or ()):
        ...
        result=insert_offset_group_fn(node,name=group_name)
        results.append({"object":node,"status":"applied","group":result["group"],"node":result["node"],"group_name":group_name})
    return tuple(results)


def create_offsets(items,insert_offset_group_fn=None,exists_fn=None):
    return execute_create_offset_plan(build_create_offset_plan(items),insert_offset_group_fn=insert_offset_group_fn,exists_fn=exists_fn)
...
def create_offset_managed_maya_smoke():
    ...
    applied=apply_create_offset((first,second),"_SceneOffset")
    named=all(item["status"]=="applied" and cmds.objExists(item["group_name"]) for item in applied)
    preserved=all(tuple(cmds.xform(node,query=True,worldSpace=True,matrix=True))==before[node] for node in (first,second))
    third=cmds.group(empty=True,name="Third_CTRL",parent=parent); fallback=apply_create_offset((third,),"")[0]
    fallback_ok=fallback["status"]=="applied" and fallback["group_name"]=="Third_CTRLExtraName" and cmds.objExists("Third_CTRLExtraName")
    missing=apply_create_offset(("Missing_CTRL",),"_SceneOffset")[0]; missing_ok=missing["status"]=="skipped_missing"
    smoke={"named":named,"world_matrix_preserved":preserved,"fallback":fallback_ok,"missing":missing_ok,"success":bool(named and preserved and fallback_ok and missing_ok)}
