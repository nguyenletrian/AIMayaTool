...
def run_skirt_parent_first_transfer_source_query_smoke():
    import importlib
    import math
    ...
    mesh = cmds.polyCylinder(name='AIMayaToolSkirtFirstTransferSourceQueryMesh', radius=2.0, height=3.0, subdivisionsX=8, subdivisionsY=2, subdivisionsZ=1)[0]
    ...
    parent = cmds.createNode('joint', name='AIMayaToolSkirtFirstTransferSourceQueryParent')
    ...
    for index, angle in enumerate((0.0, math.pi * 0.5, math.pi, math.pi * 1.5)):
        joint = cmds.createNode('joint', name='AIMayaToolSkirtFirstTransferSourceQueryJoint%d' % index)
    ...
    if not cmds.objExists(mesh):
        raise RuntimeError('Expected mesh to exist after source-only skinPercent query')
    return 'SKINNING_SKIRT_PARENT_FIRST_TRANSFER_SOURCE_QUERY_SMOKE_OK'
