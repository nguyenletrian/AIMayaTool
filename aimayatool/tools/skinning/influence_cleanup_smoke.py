...
def run_influence_cleanup_smoke():
    ...
    target_skin = cmds.skinCluster([joint_a, joint_unused], target, toSelectedBones=True, normalizeWeights=1, name='AIMayaToolInfluenceSyncTargetSkin')[0]
    ...
    cmds.skinPercent(target_skin, target_vertex, transformValue=[(joint_a, 1.0), (joint_unused, 0.0)], normalize=True)
    ...
    # The sync operation intentionally adds missing influences at zero weight.
    # Author a real target weight for joint_b before cleanup so it is a used
    # influence, leaving only joint_unused eligible for removal.
    cmds.skinPercent(target_skin, target_vertex, transformValue=[(joint_a, 0.75), (joint_b, 0.25), (joint_unused, 0.0)], normalize=True)
