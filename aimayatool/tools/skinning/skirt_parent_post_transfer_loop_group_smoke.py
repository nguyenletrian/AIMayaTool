...
def run_skirt_parent_post_transfer_loop_group_smoke():
    import importlib
    import math
    ...
    mesh = cmds.polyCylinder(name='AIMayaToolSkirtPostTransferLoopGroupMesh', radius=2.0, height=3.0, subdivisionsX=8, subdivisionsY=2, subdivisionsZ=1)[0]
    ...
    parent = cmds.createNode('joint', name='AIMayaToolSkirtPostTransferLoopGroupParent')
    ...
    for index, angle in enumerate((0.0, math.pi * 0.5, math.pi, math.pi * 1.5)):
        joint = cmds.createNode('joint', name='AIMayaToolSkirtPostTransferLoopGroupJoint%d' % index)
    ...
    if not spans or not spans[0].get('vertices'):
        raise RuntimeError('Expected first smoothing span vertices')
    strips = loop_groups.group_vertices_by_perpendicular_loops(mesh, spans[0]['vertices'])
    if not strips:
        raise RuntimeError('Expected non-empty first-span perpendicular strips after transfer')
    return 'SKINNING_SKIRT_PARENT_POST_TRANSFER_LOOP_GROUP_SMOKE_OK'
