...
def run_skin_io_smoke():
    ...
    try:
        ...
        imported_skin = skin_io.import_skin(mesh, directory)
        ...
        cmds.delete(imported_skin)
        rebound_skin = skin_io.import_skin(mesh, directory)
        rebound_a = float(cmds.skinPercent(rebound_skin, vertex, query=True, transform=joint_a) or 0.0)
        rebound_b = float(cmds.skinPercent(rebound_skin, vertex, query=True, transform=joint_b) or 0.0)
        if abs(rebound_a - 0.75) > 1e-4 or abs(rebound_b - 0.25) > 1e-4:
            raise RuntimeError('skin import did not recreate skinCluster weights: A=%s B=%s' % (rebound_a, rebound_b))
    finally:
        shutil.rmtree(directory, ignore_errors=True)
    return 'SKINNING_SKIN_IO_SMOKE_OK'
