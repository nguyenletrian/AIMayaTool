...
def run_skirt_parent_first_transfer_cmds_probe_smoke():
    import importlib
    import math
    ...
    mesh = cmds.polyCylinder(name='AIMayaToolSkirtFirstTransferCmdsProbeMesh', radius=2.0, height=3.0, subdivisionsX=8, subdivisionsY=2, subdivisionsZ=1)[0]
    ...
    parent = cmds.createNode('joint', name='AIMayaToolSkirtFirstTransferCmdsProbeParent')
    ...
    for index, angle in enumerate((0.0, math.pi * 0.5, math.pi, math.pi * 1.5)):
        joint = cmds.createNode('joint', name='AIMayaToolSkirtFirstTransferCmdsProbeJoint%d' % index)
    ...
    if len(assignments) != 4:
        raise RuntimeError('Expected four transfer assignments, got %d' % len(assignments))
    first_plan = dict(plan)
    first_plan['assignments'] = assignments[:1]
    transfers = skirt_parent_transfer.apply_parent_transfers(skin_cluster, first_plan, normalize=True)
    if len(transfers) != 1 or not transfers[0].get('changed'):
        raise RuntimeError('Expected one changed first transfer result')

    if not cmds.objExists(mesh):
        raise RuntimeError('Expected mesh to exist after first parent transfer')
    return 'SKINNING_SKIRT_PARENT_FIRST_TRANSFER_CMDS_PROBE_SMOKE_OK'
