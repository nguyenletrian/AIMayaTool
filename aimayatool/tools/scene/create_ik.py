...
from aimayatool.tools.setup.spaces import create_space_switch
...
def build_create_ik_plan(objects,parent,world_parent=None,positions=None):
    ...
    if any(not node for node in objects): raise ValueError("CreateIK object names cannot be empty.")
    if len(set(objects))!=3: raise ValueError("CreateIK requires 3 unique objects.")
    a,b,c=objects
...
def build_create_ik(objects,parent,world_parent=None):
    ...
    if len(objects)!=3 or len(set(objects))!=3: return {"status":"skipped_invalid_objects","objects":objects}
    ...
    pole=create_control(plan["pole_control"],shape="box",size=2.0); pole_offset=create_zero_group(pole,suffix="_GrpOffset")[0]; cmds.xform(pole_offset,worldSpace=True,translation=plan["frame"]["pole_position"]); cmds.parent(pole_offset,system)
    ...
    spaces=[]
    if world_parent:
        spaces.append(create_space_switch(ik_ctrl,[parent,world_parent],attr_name="Space",labels=["Local","World"],maintain_offset=True))
        spaces.append(create_space_switch(pole,[parent,world_parent],attr_name="Space",labels=["Local","World"],maintain_offset=True))
