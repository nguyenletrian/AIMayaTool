...
def run_skirt_parent_post_transfer_dagpath_smoke():
    import importlib
    import math
    ...
    mesh = cmds.polyCylinder(name='AIMayaToolSkirtPostTransferDagPathMesh', radius=2.0, height=3.0, subdivisionsX=8, subdivisionsY=2, subdivisionsZ=1)[0]
    ...
    parent = cmds.createNode('joint', name='AIMayaToolSkirtPostTransferDagPathParent')
    ...
    for index, angle in enumerate((0.0, math.pi * 0.5, math.pi, math.pi * 1.5)):
        joint = cmds.createNode('joint', name='AIMayaToolSkirtPostTransferDagPathJoint%d' % index)
    ...
    post_selection = om.MSelectionList()
    post_selection.add(mesh)
    dag_path = post_selection.getDagPath(0)
    if dag_path.isValid() is False:
        raise RuntimeError('Expected valid post-transfer DAG path')
    return 'SKINNING_SKIRT_PARENT_POST_TRANSFER_DAGPATH_SMOKE_OK'
