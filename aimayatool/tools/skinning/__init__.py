...
def build_ui():
    ...
    section('Influence management', spacing=False)
    ...
    section('Max influences (uses skinCluster setting)')
    cmds.rowLayout(numberOfColumns=2, adjustableColumn=2, columnWidth2=(190, 190))
    ...
    section('Skin transfer', 'Select source mesh first, then one or more targets.')
    cmds.rowLayout(numberOfColumns=2, adjustableColumn=2, columnWidth2=(190, 190))
    ...
    section('Skirt parent workflow', 'Select parent joint first, skirt joints, then root-loop edges.')
    cmds.button(label='Run Skirt Parent', command=lambda *_: _run('Skirt parent', skirt_parent_interactive.run_from_selection))

    section('Mirror skin across X / YZ plane')
    cmds.rowLayout(numberOfColumns=2, adjustableColumn=2, columnWidth2=(190, 190))
    ...
    section('Paint Skin Weights', 'Activate Maya Paint Skin Weights for active-influence navigation.')
    ...
    section('Proxy skin', 'Select polygon faces from one skinned mesh.')
    cmds.rowLayout(numberOfColumns=2, adjustableColumn=2, columnWidth2=(190, 190))
    ...
    section('Selection set navigation')
    cmds.rowLayout(numberOfColumns=4, adjustableColumn=4, columnWidth4=(95, 95, 95, 95))
    ...
    section('Skin data', 'Preview validates selected meshes and planned skinCluster behavior before running.')
    cmds.rowLayout(numberOfColumns=2, adjustableColumn=2, columnWidth2=(190, 190))
